...
export async function POST(request:NextRequest){
  ...
  const login=body.login?.trim().toLowerCase()??"",password=body.password??"",ipKey=requestIpHash(request)??"unknown",rateKey=`${ipKey}:${login}`;
  ...
  const result=await query<AuthUser&{passwordHash:string}>(`SELECT "id","username","email","passwordHash","emailVerifiedAt","role","sessionGeneration" FROM "User" WHERE "username"=$1 OR "email"=$1 LIMIT 1`,[login]);
  ...
  if(settings.emailVerificationRequired&&!user.emailVerifiedAt){
    await query(`INSERT INTO "SecurityEvent" ("id","userId","type","ipHash","createdAt") VALUES ($1,$2,'login.unverified',$3,NOW())`,[randomUUID(),user.id,requestIpHash(request)]);
    return NextResponse.json({error:"Verify your email address before signing in.",code:"EMAIL_UNVERIFIED"},{status:403});
  }
