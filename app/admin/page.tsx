...
export default async function AdminPage(){const user=await currentUser();if(!user)redirect("/");if(user.role!=="ADMIN")redirect("/workspace");const [settings,emailSettings,users]=await Promise.all([getSiteSettings(),getEmailAdminView(),query<{id:string;username:string;email:string;role:string;emailVerifiedAt:Date|null;createdAt:Date}>(`SELECT "id","username","email","role","emailVerifiedAt","createdAt" FROM "User" ORDER BY "createdAt","username"`)]);return <AdminPanel currentUserId={user.id} initialSettings={settings} initialEmail={emailSettings} initialUsers={users.rows.map(item=>({...item,emailVerifiedAt:item.emailVerifiedAt?.toISOString()??null,createdAt:item.createdAt.toISOString()}))}/>}
