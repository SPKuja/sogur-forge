...
type ChapterRow={id:string;partId:string|null;title:string;kind:string;pageType:string|null;content:string;summary:string;status:string;position:number;templateId:string|null;headerImageAssetId:string|null};
export default async function Page({params}:{params:Promise<{novelId:string}>}){
  ...
  const [chapters,parts,templates]=await Promise.all([
    query<ChapterRow>(`SELECT "id","partId","title","kind","pageType","content","summary","status","position","templateId","headerImageAssetId" FROM "Chapter" WHERE "novelId"=$1 ORDER BY "position"`,[novelId]),
