...
export default function CorkBoard({username,novel,initialBoards,initialNotes}:{username:string;novel:{id:string;title:string};initialBoards:Board[];initialNotes:Note[]}){
  ...
  async function deleteNote(id:string){const r=await fetch(`/api/sticky-notes/${id}`,{method:"DELETE"});if(r.ok)setNotes(v=>v.filter(x=>x.id!==id))}
  ...
  return <div className="app section-shell"><WorkspaceSectionSidebar username={username} novel={novel} active="cork-board" open={navOpen} onClose={()=>setNavOpen(false)}/>{navOpen&&<button className="scrim" onClick={()=>setNavOpen(false)}/>}<main className="section-main cork-page">
    ...
    <div className="cork-canvas" onPointerMove={move} onPointerUp={up} onPointerCancel={up} onDoubleClick={e=>{if(e.target!==e.currentTarget)return;const r=e.currentTarget.getBoundingClientRect();addNote(e.clientX-r.left-110,e.clientY-r.top-40)}}>{notes.filter(n=>n.boardId===active).map(n=><article key={n.id} className={`sticky sticky-${n.color}`} style={{left:n.positionX,top:n.positionY,width:n.width,height:n.height}}><div className="sticky-pin" onPointerDown={e=>{if((e.target as HTMLElement).closest("button"))return;const el=e.currentTarget.parentElement!,r=el.getBoundingClientRect();drag.current={id:n.id,dx:e.clientX-r.left,dy:e.clientY-r.top};e.currentTarget.setPointerCapture(e.pointerId)}}>● <span>drag</span><button aria-label="Delete note" onPointerDown={e=>e.stopPropagation()} onClick={e=>{e.stopPropagation();deleteNote(n.id)}} onDoubleClick={e=>e.stopPropagation()}>×</button></div><textarea autoFocus={!n.body} placeholder="Write something…" value={n.body} onChange={e=>setNotes(v=>v.map(x=>x.id===n.id?{...x,body:e.target.value}:x))} onBlur={()=>save(notes.find(x=>x.id===n.id)!)} /><div className="sticky-colors">{["yellow","blue","pink","green"].map(c=><button aria-label={c} key={c} className={c} onClick={()=>{const nn={...n,color:c};setNotes(v=>v.map(x=>x.id===n.id?nn:x));save(nn)}} />)}</div></article>)}{!notes.some(n=>n.boardId===active)&&<div className="cork-empty"><strong>Your board is empty</strong><span>Double-click anywhere or add a sticky.</span></div>}</div>
  </main></div>
}
