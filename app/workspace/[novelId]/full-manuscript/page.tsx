  ...
  return <ManuscriptLayoutProvider novelId={novelId} initialLayout={layoutBundle.layout} initialDisplayMode={layoutBundle.displayMode}><div className="full-manuscript-shell">
    ...
    <div className="full-manuscript-layout">
      ...
      <main className="full-manuscript-main">
        <div className="full-manuscript-paper">
          {ordered.map((item,index)=>{
            ...
            return <div key={item.id}>
              ...
              <article id={`item-${item.id}`} className="full-manuscript-item">
                ...
                {scenes.length?scenes.map((scene,sceneIndex)=><div id={`scene-${scene.id}`} className="full-manuscript-scene" key={scene.id}>{sceneIndex>0&&<div className="full-scene-break" aria-label="Scene break">* * *</div>}{scenes.length>1&&<div className="full-scene-label"><span>Scene {sceneIndex+1}{scene.title?` · ${scene.title}`:""}</span><Link href={`${editorHref}&scene=${scene.id}`}>Edit scene ↗</Link></div>}<div className="rich-editor full-manuscript-prose" dangerouslySetInnerHTML={{__html:cleanManuscriptHtml(scene.content)}}/></div>):<div className="rich-editor full-manuscript-prose" dangerouslySetInnerHTML={{__html:cleanManuscriptHtml(item.content)}}/>}
              </article>
            </div>
          })}
        </div>
