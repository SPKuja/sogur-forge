...
function transform(value:string):"uppercase"|"lowercase"|"none"{return value==="UPPER"?"uppercase":value==="LOWER"?"lowercase":"none"}
...
export default function ChapterTemplateHeader({template,context,imageUrl,className=""}:{template:ChapterTemplateDesign;context:ChapterTemplateContext;imageUrl?:string|null;className?:string}){
  ...
  const label=template.eyebrowPattern.trim()?<div key="label" className="chapter-template-label" style={{textAlign:align(template.labelAlign),fontSize:template.labelSize,fontWeight:template.labelWeight,fontFamily:font(template.labelFont),marginBottom:template.labelSpacing,color:template.labelColor||"var(--accent)",textTransform:transform(template.labelCase)}}>{renderChapterTemplateText(template.eyebrowPattern,context)}</div>:null;
  const title=template.titlePattern.trim()?<div key="title" className="chapter-template-title" style={{textAlign:align(template.titleAlign),fontSize:template.titleSize,fontWeight:template.titleWeight,fontFamily:font(template.titleFont),marginBottom:template.titleSpacing,color:template.titleColor||"var(--ink)",textTransform:transform(template.titleCase)}}>{renderChapterTemplateText(template.titlePattern,context)}</div>:null;
