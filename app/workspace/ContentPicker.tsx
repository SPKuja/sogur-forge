...
export type PickerChapter={id:string;partId:string|null;title:string;position:number};
...
export default function ContentPicker({novelTitle,parts,chapters,selectedIds,onChange,onClose}:{novelTitle:string;parts:PickerPart[];chapters:PickerChapter[];selectedIds:string[];onChange:(ids:string[])=>void;onClose:()=>void}){
  ...
  const groups=useMemo(()=>{
    ...
    return loose.length?[{id:"",title:"Unassigned chapters",chapters:loose},...ordered]:ordered;
  },[parts,chapters]);
  ...
  return <div className="modal-backdrop content-picker-backdrop" onMouseDown={onClose}><section className="content-picker" role="dialog" aria-modal="true" aria-labelledby="content-picker-title" onMouseDown={e=>e.stopPropagation()}>
    <header><div><small>EXPORT</small><h2 id="content-picker-title">Choose manuscript content</h2><p>Select the Parts and Chapters you want to include.</p></div><button aria-label="Close" onClick={onClose}>×</button></header>
    <div className="content-picker-actions"><span>{selected.size} of {chapters.length} chapters selected</span><button onClick={()=>setSelection(new Set(chapters.map(c=>c.id)))}>Select all</button><button onClick={()=>setSelection(new Set())}>Clear all</button></div>
    <div className="content-tree">
      <label className="content-root"><TriCheckbox checked={allSelected} mixed={selected.size>0&&!allSelected} onChange={toggleAll} label={`Select all of ${novelTitle}`}/><strong>{novelTitle}</strong><small>{chapters.length} chapters</small></label>
      {groups.filter(g=>g.chapters.length).map(g=>{const ids=g.chapters.map(c=>c.id),count=ids.filter(id=>selected.has(id)).length,all=count===ids.length,closed=collapsed.has(g.id||"__loose");return <section className="content-group" key={g.id||"loose"}>
        ...
        {!closed&&<div className="content-children">{g.chapters.map(c=><label key={c.id}><input type="checkbox" checked={selected.has(c.id)} onChange={()=>toggleChapter(c.id)}/><span>{c.title}</span></label>)}</div>}
      </section>})}
    </div>
