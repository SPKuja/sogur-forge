...
export type ExportChapter={id:string;title:string;position:number;partId:string|null;partTitle:string|null;partPosition:number|null;contentHtml:string};
...
export async function buildManuscriptExport(userId:string,novelId:string,chapterIds:string[],options:ExportOptions):Promise<ManuscriptExport|null>{
  ...
  const chapters=await query<{id:string;title:string;content:string;position:number;partId:string|null;partTitle:string|null;partPosition:number|null}>(`SELECT c."id",c."title",c."content",c."position",c."partId",p."title" AS "partTitle",p."position" AS "partPosition" FROM "Chapter" c LEFT JOIN "Part" p ON p."id"=c."partId" WHERE c."novelId"=$1 AND c."id"=ANY($2::text[]) ORDER BY c."position"`,[novelId,unique]);
  return {novel:novel.rows[0],options,chapters:chapters.rows.map(c=>({id:c.id,title:c.title,position:c.position,partId:c.partId,partTitle:c.partTitle,partPosition:c.partPosition,contentHtml:cleanInternalMarkup(c.content)}))};
}
